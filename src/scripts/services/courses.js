/**
 * api/courses provider. Used in order to minimize api requests for pulling courses.
 * This way we can quickly expose all (~2000) courses to the application.
 * Search and everything is done on the frontend! Makes the backend much leaner.
 */
angular.module('services.courses', []).factory('Courses', function(
	$q,
	$filter,
	Restangular,
	_
	...
	self.fetch = function(regulation, lower, upper, student) {
		...
		$q.all( promises ).then(function(c) {
			...
			var a = $filter('courseFilter')(courses[reg], { 'year#<&&year#>': (lower-1)+'&&'+(upper+1) });
			deferred2.resolve(a);
		});
