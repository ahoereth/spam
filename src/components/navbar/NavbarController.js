...
export default class NavbarController {
  ...
  $onInit() {
    ...
    this.$rootScope.$watch('loading', n => {
      if (n > 0) {
        ...
        hideLoader = this.$timeout(() => { this.showLoader = false; }, 1100);
      }
    });

    // Watch the quick search input field for changes.
    this.$scope.$watch(() => this.search.query, (query, oldQuery) => {
    ...
    this.$scope.$watch(() => this.search.focus, focused => {
      if (focused === false) {
    ...
    this.$scope.$watch(() => this.search.hover, hovering => {
  ...
  applyFilter() {
    ...
    this.search.filtered =
      this.$filter('courseFilter')(this.courses, this.search.filter)
  ...
  fetch() {
    if (this.lowerYear < 1995 || this.fetching) { return; }

    if (this.search.limit > this.search.filtered.length) {
      this.fetching = true;
      this.Courses
        .fetch(this.User.getRegulation(1), this.lowerYear, this.upperYear, null)
        .then(newCourses => {
  ...
  searchBoxToggle(happening) {
    ...
    switch (happening) { // eslint-disable-line default-case
      case 'mouseover': this.states.mouse = true; break;
      case 'mouseout': this.states.mouse = false; break;
      case 'focus': this.states.input = true; break;
      case 'blur': this.states.input = false; break;
    }
